...
func _run() -> void:
	...
	game._night_puzzle_ui._validating = true
	game._night_puzzle_ui._show_fail_panel()
	...
	_check(not game._night_puzzle_ui._validating, "Wrong tutorial Finalize must unlock a fresh assignment attempt.")
	_check(not game._night_puzzle_ui.get_node("%FailPanel").visible, "Tutorial retry guidance must replace the normal fail panel.")
	tutorial._complete_typewriter()
	tutorial._advance_from_continue()
