...
func _refresh_availability(animate: bool) -> void:
	...
	if should_show:
		...
		if animate:
			_set_drawer_x(_drawer_hidden_x())
			...
			_drawer_tween.tween_property(self, ^"self_modulate:a", 1.0, drawer_duration * 0.7).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		else:
			_set_drawer_x(collapsed_x)
		return
	if not visible:
...
func _slide_out_and_hide() -> void:
	...
	_drawer_tween = create_tween()
	_drawer_tween.set_parallel(true)
	_drawer_tween.tween_property(_tray, ^"position:x", _drawer_hidden_x(), drawer_duration).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_IN)
...
func _on_choice_hover(choice: Control, hovered: bool) -> void:
	if _dragging or _committed or _stamp_locked or not _ticket_is_visible or not _expanded:
		return
	...
	# Hover may interrupt the entry tween. Always finish at the shared baseline
	# so one stamp can never remain stranded above or below its neighbours.
	tween.tween_property(choice, ^"position", rest_position, 0.12).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func _animate_choices_in() -> void:
	for station_name: String in STATIONS:
		...
		_choice_tweens[choice] = tween
		tween.set_parallel(true)
		tween.tween_property(choice, ^"position", rest_position, choice_entry_duration).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
		tween.tween_property(choice, ^"scale", Vector2.ONE, choice_entry_duration).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tween.tween_property(choice, ^"self_modulate", Color.WHITE, choice_entry_duration * 0.72).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func _drawer_hidden_x() -> float:
	return -_tray.size.x - 36.0
