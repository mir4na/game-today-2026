extends Control
## Circular off-screen pointer for a scene-authored maintenance event target.
...
@export_range(0.0, 120.0, 1.0) var camera_visibility_margin: float = 12.0
...
func set_target(target: Node2D, tracker_icon: Texture2D) -> void:
...
func _update_screen_position() -> void:
	...
	var camera_rect := Rect2(Vector2.ZERO, viewport_size).grow(-camera_visibility_margin)
	if camera_rect.has_point(target_screen_position):
		hide()
		return
	show()
	...
	var pointer_rotation: float = direction.angle() - PI * 0.5
	...
	if is_instance_valid(_pointer):
		var pointer_center: Vector2 = size * 0.5 + direction * (size.x * 0.42)
		_pointer.position = pointer_center - _pointer.size * 0.5
